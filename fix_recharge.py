...
# It looks like my previous regex replacement failed silently because the exact signature 
# `@Composable\s*fun DataRechargeDialog\s*\(.*?^}\s*}(?=\n\n|\Z)` didn't match. 
# The command output shows it's still using the dark red UI (Color(0xFF140508) etc).
# Let's replace the whole dialog function more precisely.

old_func_start = "fun DataRechargeDialog("
# Find the start of the function and replace until the end of it.
# We will just replace it using regex from `@Composable\s*fun DataRechargeDialog` down to the next `//` or EOF
...
content = re.sub(r'@Composable\s*fun DataRechargeDialog.*?(?=\Z|@Composable)', new_dialog, content, flags=re.MULTILINE | re.DOTALL)
