...
for i, line in enumerate(lines):
    if line.startswith('fun DataRechargeCard'):
        start_idx = i - 1 # @Composable is one line before
        break

if start_idx != -1:
    for i in range(start_idx + 1, len(lines)):
        if line.startswith('fun DataRechargeDialog'):
            break
        if lines[i].startswith('fun DataRechargeDialog'):
            # step back to find }
